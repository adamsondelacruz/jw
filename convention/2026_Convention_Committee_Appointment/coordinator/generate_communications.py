...
def people(values):
    return ", ".join(f'<a href="mailto:{quote(p["email"])}">{escape(p["name"])} &lt;{escape(p["email"])}&gt;</a>' for p in values) or "—"
...
for record in data["communications"]:
    nav.append(f'<button class="comm-select" data-target="{escape(record["id"])}"><span>{escape(record["department"])}</span><small>{escape(record["to"][0]["name"])}</small><b>{escape(record["status"])}</b></button>')
    attachments = []
...
<header><div><p class="eyebrow">Department communication</p><h2>{escape(record["department"])}</h2></div><span class="status">{escape(record["status"])}</span></header>
...
body{{max-width:1500px}}.comm-shell{{display:grid;grid-template-columns:300px minmax(0,1fr);gap:1.25rem;align-items:start}}.comm-sidebar{{position:sticky;top:4.4rem;max-height:calc(100vh - 5.5rem);overflow:auto;padding:.75rem;border:1px solid var(--line);border-radius:.7rem;background:var(--wash)}}.comm-sidebar h2{{margin:.2rem .4rem .7rem;font-size:1rem}}.comm-select{{display:grid;width:100%;margin:.35rem 0;padding:.65rem .7rem;border:1px solid transparent;border-radius:.45rem;background:#fff;color:var(--ink);font:inherit;text-align:left;cursor:pointer}}.comm-select span{{font-weight:800}}.comm-select small{{color:var(--muted)}}.comm-select b{{margin-top:.25rem;color:#805b10;font-size:.72rem;text-transform:uppercase}}.comm-select.active{{border-color:var(--blue);background:var(--blue-pale)}}.comm-detail{{display:none;padding:1.2rem;border:1px solid var(--line);border-radius:.75rem;background:#fff}}.comm-detail.active{{display:block}}.comm-detail>header{{display:flex;justify-content:space-between;gap:1rem;align-items:start;border-bottom:1px solid var(--line)}}.eyebrow{{margin:0;color:var(--muted);font-size:.78rem;font-weight:800;text-transform:uppercase;letter-spacing:.08em}}.comm-detail h2{{margin:.15rem 0 1rem}}.status{{padding:.35rem .65rem;border-radius:999px;background:#fff3cd;color:#664d03;font-weight:800;white-space:nowrap}}dl{{display:grid;grid-template-columns:8rem 1fr;gap:.45rem 1rem}}dt{{font-weight:800}}dd{{margin:0;overflow-wrap:anywhere}}details{{margin:1rem 0;border:1px solid var(--line);border-radius:.5rem}}summary{{padding:.65rem .8rem;background:var(--wash);font-weight:800;cursor:pointer}}summary span{{float:right}}.attachments{{margin:0;padding:.5rem 1rem;list-style:none}}.attachments li{{display:flex;justify-content:space-between;gap:1rem;padding:.4rem 0;border-top:1px solid var(--line)}}.attachments li:first-child{{border-top:0}}.attachments span{{color:#1f6f32;font-size:.8rem;font-weight:800}}.attachments .missing span{{color:#9b2c2c}}.email-body{{padding:1rem 1.2rem;max-width:820px;background:#fff}}.record-note{{padding:.7rem .8rem;border-left:4px solid #d69e2e;background:#fffaf0}}.dry-run{{padding:.8rem 1rem;border:2px solid #d69e2e;border-radius:.55rem;background:#fffaf0;font-weight:750}}@media(max-width:800px){{.comm-shell{{grid-template-columns:1fr}}.comm-sidebar{{position:static;max-height:none}}dl{{grid-template-columns:1fr}}dt{{margin-top:.4rem}}}}
</style></head><body><nav class="site-nav"><a href="index.html">Index</a><a href="organisation-chart.html">Chart</a><a href="contact-masterlist.html">Contacts</a><a href="communications.html" aria-current="page">Communications</a></nav>
...
<div class="comm-shell"><aside class="comm-sidebar"><h2>Recipients and status</h2>{''.join(nav)}</aside><main>{''.join(cards)}</main></div>
<script>const buttons=[...document.querySelectorAll('.comm-select')],cards=[...document.querySelectorAll('.comm-detail')];function show(id){{buttons.forEach(b=>b.classList.toggle('active',b.dataset.target===id));cards.forEach(c=>c.classList.toggle('active',c.id===id));history.replaceState(null,'','#'+id)}}buttons.forEach(b=>b.addEventListener('click',()=>show(b.dataset.target)));show(location.hash.slice(1)&&document.getElementById(location.hash.slice(1))?location.hash.slice(1):buttons[0].dataset.target);addEventListener('hashchange',()=>{{if(document.getElementById(location.hash.slice(1)))show(location.hash.slice(1))}});</script></body></html>'''
